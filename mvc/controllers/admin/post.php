...
if (isset($_POST['them_sp'])) {
    ...
    if (empty($title) || empty($content) || empty($image) || empty($quantity) || empty($price) || empty($thongtin)) {
        $error['one'] = "<script>alert('Bạn nhập thông tin chưa đầy đủ');</script>";
    }
    if (empty($error)) {
    ...
    header("Location: /thoitrangTNT/mvc/controllers/admin.php?action=danhsachsp");
    exit;
}
...
if (isset($_POST['check_delete_info'])) {
    $id_check = $_POST['checkbox'];
    foreach ($id_check as $val) {
...
if (isset($_POST['check_delete_order'])) {
    $id_check = $_POST['checkbox'];
    foreach ($id_check as $val) {
        $post->delete('orders', 'id_order', $val);
        // setcookie('error',"<script>alert('Bạn xoá thành công');</script>",time() + 48000);
    }
}
