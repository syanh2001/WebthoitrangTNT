                            ...
                            foreach ($post->get_val('news') as $val) : extract($val);?>
                                <form action="" method="post">
                                    <tr>
                                        ...
                                        <td><?= $date ?></td>
                                        <td><?= $author ?></td>
                                        <td><a href="<?=CONTROLLERS_ADMIN?>?action=sua_new&id_new=<?php echo $id_news ?>"><button type="button" class="btn btn-warning">Sửa</button></a></td>
                                    </tr>
                            <?php endforeach;
                        } ?>
                    </tbody>
                </table>
                <input onclick="return confirm('Bạn muốn xóa theo lựa chọn ?');" class="btn btn-danger mt-3" type="submit" name="check_delete_new" value="xóa click" id="checkall">
                </form>
            </div>
        </main>